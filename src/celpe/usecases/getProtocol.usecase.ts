...
export class GetProtocolUseCase {
    ...
    async execute(loginData: LoginRequest, uc: string): Promise<ProtocolResponse | null> {
        ...
        if (payload.sub) {
            try {
                return await this.externalApiService.getUcProtocol( uc ,token.token.ne, payload.sub);
            } catch (error) {
